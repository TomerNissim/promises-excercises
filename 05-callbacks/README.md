...
If we have a user, we will call the callback as `cb(null, user)`. If we don't have a user, we will wait 1000 ms and then call the callback with a my message for the error param, e.g. `cb('User Not Found!')`
